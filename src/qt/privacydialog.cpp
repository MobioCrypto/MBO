                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMBO ought to be enough for anybody." - Bill Gates, 2017
    ui->zMBOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMBOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMBO->setEnabled(false);
        ui->pushButtonMintzMBO->setToolTip(tr("zMBO is currently disabled due to maintenance."));

        ui->pushButtonSpendzMBO->setEnabled(false);
        ui->pushButtonSpendzMBO->setToolTip(tr("zMBO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMBOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMBO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMBO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMBO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMBO in ") +
...
void PrivacyDialog::on_pushButtonSpendzMBO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMBO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMBO
        sendzMBO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMBO
    sendzMBO();
}

void PrivacyDialog::on_pushButtonZMobioControl_clicked()
{
    ZMobioControlDialog* zMobioControl = new ZMobioControlDialog(this);
    zMobioControl->setModel(walletModel);
    zMobioControl->exec();
}

void PrivacyDialog::setZMobioControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMobioSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMBO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Mobio Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMBOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMBO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MBO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMBO</b>";
    ...
    // use mints from zMobio selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMobioControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMobioControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmobio selector in case it was used
    ZMobioControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMobio Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Mobio, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMobio Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMBO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMBO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMBO "));
    ui->labelzMBOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMBOSyncStatus->setVisible(fShow);
}
