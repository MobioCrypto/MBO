...
#include "ui_zmobiocontroldialog.h"
...
std::list<std::string> ZMobioControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMobioControlDialog::listMints;

ZMobioControlDialog::ZMobioControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMobioControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMobioControlDialog::~ZMobioControlDialog()
{
    delete ui;
}

void ZMobioControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMobioControlDialog::updateList()
{
...
void ZMobioControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMobioControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMobio_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMobioControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMobioControlDialog::GetSelectedMints()
{
...
void ZMobioControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
