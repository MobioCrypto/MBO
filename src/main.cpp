...
#if defined(NDEBUG)
#error "Mobio cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 90 * 60;
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMobio spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zMobio spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zMobio spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	nSubsidy = 6250000 * COIN;
        ...
        nSubsidy = 60 * COIN;
    } else if (nHeight <= 150000 && nHeight > 50000) {
        nSubsidy = 35 * COIN;
    } else if (nHeight <= 200000 && nHeight > 150000) {
        nSubsidy = 25 * COIN;
    } else if (nHeight <= 300000 && nHeight > 200000) {
        nSubsidy = 20 * COIN;
    } else if (nHeight <= 416101 && nHeight > 300000) {
        nSubsidy = 15 * COIN;
    } else if (nHeight <= 900000 && nHeight > 416101) {
        nSubsidy = 7.5 * COIN;
    } else if (nHeight <= 1300000 && nHeight > 900000) {
        nSubsidy = 4 * COIN;
    } else if (nHeight <= 1600000 && nHeight > 1300000) {
        nSubsidy = 2 * COIN;
    } else {
        nSubsidy = 1 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Mobio
...
{
    RenameThread("mobio-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMBOMinted()
{
...
void RecalculateZMBOSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMBO supply
...
bool RecalculateMBOSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zMobio with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMobioSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MBO to zMBO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMobio spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMobio serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMobio mints and %d zMobio spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Mobio: We use certain sporks during IBD, so check to see if they are
